...
import cn.enilu.flash.mobile.wx.fastweixin.api.response.GetSignatureResponse;
import cn.enilu.flash.mobile.wx.fastweixin.util.BeanUtil;
import cn.enilu.flash.mobile.wx.fastweixin.util.JsApiUtil;
...
public class JsAPI extends BaseAPI {
    ...
    public GetSignatureResponse getSignature(String nonceStr, long timestame, String url) {
        ...
        try {
            ...
            LOG.debug("jsApiticket：{} , sign：{}",jsApiTicket, sign);
            LOG.debug("nonceStr：{} , timestame：{} , url：{}",nonceStr,timestame,url);
        } catch (Exception e) {
            LOG.error("获取签名异常:", e);
